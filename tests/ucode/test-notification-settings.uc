...
let settings = {
	...
	luci_enabled: true,
	luci_events: [ 'recovery', 'miclash_event' ],
...
assert_json_equal(configured.luci.types, [ 'recovery', 'miclash_event' ]);
...
assert_throws(() => notification_settings.notifier_config({ ...settings,
	syslog_events: [ 'miclash_event' ] }), 'INVALID_ARGUMENT');
assert_throws(() => notification_settings.telegram_config({ ...settings,
	telegram_events: [ 'miclash_event' ] }), 'INVALID_ARGUMENT');
assert_throws(() => notification_settings.telegram_config({ ...settings,
