...
```bash
apk update
apk add zlib libcurl4 curl kmod-nft-tproxy kmod-nft-nat kmod-tun coreutils-base64
...
```bash
opkg update && opkg install zlib libcurl4 curl kmod-nft-tproxy kmod-nft-nat kmod-tun coreutils-base64
release=$(curl -s https://api.github.com/repos/ang3el7z/luci-app-miclash/releases/latest | grep '"tag_name"' | head -n1 | cut -d '"' -f4)
